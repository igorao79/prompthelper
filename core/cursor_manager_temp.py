...
class CursorManager:
    ...
    def auto_paste_prompt(self, delay_seconds=5):
        ...
        """
        # ДОБАВЛЯЕМ ЖЕСТКИЙ ТРОТТЛИНГ ПЕРЕД ЛЮБЫМИ ОПЕРАЦИЯМИ С CURSOR!
        print("🛡️ ПРИМЕНЯЕМ ЖЕСТКИЙ ТРОТТЛИНГ перед операцией вставки промпта...")
        self._throttle_before_launch(self._preferred_window_hint or "unknown")
        ...
        if PYAUTOGUI_AVAILABLE:
            try:
                ...
                # Получаем текущий путь к проекту через подсказку окна
                current_project_path = self._preferred_window_hint or "unknown"
                hwnd = self._find_cursor_process_for_project(current_project_path)
                ...
                # ДОПОЛНИТЕЛЬНАЯ ВАЛИДАЦИЯ: Проверяем что окно точно правильное!
                if not self._validate_cursor_window(hwnd, self._preferred_window_hint):
                    print("❌ Ошибка: Найденное окно НЕ соответствует проекту!")
                    return False, "Неправильное окно Cursor"
    ...
    def _validate_cursor_window(self, hwnd, expected_project_hint) -> bool:
        """
        ЖЕСТКАЯ валидация окна Cursor перед операциями
        
        Args:
            hwnd: Handle окна
            expected_project_hint: Ожидаемое имя проекта
            
        Returns:
            bool: True если окно валидно, False иначе
        """
        try:
            if not hwnd or not isinstance(hwnd, int):
                print("❌ ВАЛИДАЦИЯ: Неверный HWND")
                return False
                
            if platform.system().lower() != 'windows':
                return True  # На других платформах валидация не поддерживается
                
            # Получаем заголовок окна
            buffer = ctypes.create_unicode_buffer(512)
            length = ctypes.windll.user32.GetWindowTextW(hwnd, buffer, 512)
            
            if length == 0:
                print("❌ ВАЛИДАЦИЯ: Не удалось получить заголовок окна")
                return False
                
            window_title = buffer.value
            print(f"🔍 ВАЛИДАЦИЯ: Заголовок окна: '{window_title}'")
            
            # Проверяем что это окно Cursor
            if 'cursor' not in window_title.lower():
                print(f"❌ ВАЛИДАЦИЯ: Окно НЕ является Cursor! Заголовок: '{window_title}'")
                return False
                
            # Проверяем проект если задан
            if expected_project_hint and expected_project_hint != "unknown":
                project_name = expected_project_hint.lower()
                if project_name not in window_title.lower():
                    print(f"❌ ВАЛИДАЦИЯ: Неправильный проект! Ожидался '{expected_project_hint}', найден '{window_title}'")
                    return False
                    
            # Проверяем что окно активно/видимо
            if not ctypes.windll.user32.IsWindowVisible(hwnd):
                print("❌ ВАЛИДАЦИЯ: Окно невидимо")
                return False
                
            if ctypes.windll.user32.IsIconic(hwnd):
                print("❌ ВАЛИДАЦИЯ: Окно свернуто")
                return False
                
            print(f"✅ ВАЛИДАЦИЯ: Окно '{window_title}' прошло все проверки!")
            return True
