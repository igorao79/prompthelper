...
def get_language_by_country(country):
    """Возвращает язык для страны с учетом региональной специфики"""
    language_names = {
        "ru": "Русский",
        "uk": "Украинский", 
        "be": "Белорусский",
    ...
    # Специфичные региональные названия (только где есть региональные коды)
    regional_names = {
    ...
    # Сначала ищем точное совпадение с региональным кодом
    if lang_code in regional_names:
        return regional_names[lang_code]
    
    # Если нет, извлекаем базовый код языка (es-PE → es)
    base_lang = lang_code.split('-')[0]
    return language_names.get(base_lang, "Неизвестный язык")


def get_html_lang_code(country):
    """Возвращает HTML lang код для страны (для атрибута lang в HTML)"""
    return COUNTRIES_DATA.get(country, "ru")
