#!/usr/bin/env python3
"""
ПРОСТЕЙШИЙ скрипт для создания EXE файла
Только основные функции, никаких тяжелых библиотек
"""
...
def create_minimal_main():
    """Создает минимальный main.py только для EXE"""
...
'''
    ...
    print("✅ Создан minimal_main.py")

def build_simple_exe():
    """Собирает простейший EXE"""
    print("🔨 СБОРКА ПРОСТОГО EXE...")
    
    # Только самые базовые параметры
    cmd = [
        sys.executable, '-m', 'PyInstaller',
        '--onefile',                    # Один файл
        '--windowed',                   # Без консоли
        '--name=LandingGenerator',      # Простое имя
        'minimal_main.py'               # Простой файл
    ]
...
def main():
    """Главная функция"""
    print("🚀 СОЗДАНИЕ ПРОСТОГО EXE ФАЙЛА")
    print("=" * 50)
    
    # Этапы
    steps = [
        ("Очистка", clean_everything),
        ("Создание минимального main", create_minimal_main),
        ("Сборка EXE", build_simple_exe),
    ]
